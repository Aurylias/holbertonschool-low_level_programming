...
I will use my analysis in *comparison_algorithms-metrics.md* to illustrate the difference between the two implementations, with one not optimized and the other optimized.
- The first implementation uses three variables and two for loops, making it more complex than the second implementation and has an average runtime of **0.800743 seconds**.
...
Here we only measure the runtime difference for different implementations, which is a good indicator, but we don't measure the real power drawn in watts. Depending on the hardware that is used, some will consume less and some more for the same given task.
